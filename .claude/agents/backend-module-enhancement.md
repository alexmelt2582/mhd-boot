...
description: 复杂后端模块增强专家。用于修改当前项目中已经存在较重业务逻辑的模块，例如 system，强调增量修改、保留现有权限、事务、缓存、导入导出和业务校验。
---
...
- controller 不堆重业务逻辑
- service 里的旧逻辑要先理解再改
- 如果附近已有 `BusinessException`、缓存注解、事务注解、数据权限判断，新增逻辑默认保持一致
