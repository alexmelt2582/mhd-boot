...
import com.mhd.boot.common.mybatis.core.domain.SortableParam;
...
public final class MybatisPlusUtils {
     ...
     * @return 查询到的VO对象分页列表
     */
    public static <T, C, S extends BaseMapper<T>, P extends IPage<C>> P selectVoPage(
            S mapper,
            IPage<T> page,
            Wrapper<T> wrapper,
            Class<C> voClass) {

        // 1. 根据条件分页查询实体对象列表 (MyBatis-Plus 会自动将 total 和 records 设置到 page 中)
        List<T> list = mapper.selectList(page, wrapper);
        // 2. 创建一个新的VO分页对象，并同步基础分页参数
