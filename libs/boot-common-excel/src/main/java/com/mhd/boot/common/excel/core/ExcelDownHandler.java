package com.mhd.boot.common.excel.core;
...
import com.mhd.boot.common.excel.annotation.ExcelDictFormat;
import com.mhd.boot.common.excel.annotation.ExcelDynamicOptions;
import com.mhd.boot.common.excel.annotation.ExcelEnumFormat;
            ...
            if (field.isAnnotationPresent(ExcelDictFormat.class)) {
                ...
                if (StringUtils.isNotBlank(dictType)) {
                    ...
                    Collection<String> values = Optional.ofNullable(getDictService().getAllDictByDictType(dictType))
                        .orElseThrow(() -> new BusinessException("字典 {} 不存在", dictType))
                        .values();
                    options = new ArrayList<>(values);
                } else if (StringUtils.isNotBlank(converterExp)) {
                    // 如果指定了确切的值，则直接解析确切的值
                    List<String> strList = StringUtils.splitList(converterExp, SignConstants.COMMA);
                    options = CollectionUtils.convertList(strList, s -> {
                        String[] itemArray = s.split("=", 2);
                        if (itemArray.length != 2) {
                            throw new BusinessException("Excel转换表达式格式错误: {}", s);
                        }
                        return itemArray[1];
                    });
                }
                ...
                options = CollectionUtils.convertList(values, Convert::toStr);
    ...
    private void validateLinkedOptionNames(List<String> firstOptions) {
        ...
        for (String firstOption : firstOptions) {
            ...
            if (!names.add(firstOption)) {
                throw new BusinessException("级联下拉一级选项重复：" + firstOption);
            }
        }
    }
