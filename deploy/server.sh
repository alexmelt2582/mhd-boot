...
# 检查 JAVA_HOME
if [ -z "$JAVA_HOME" ]; then
  echo "[ERROR] JAVA_HOME 未设置，请先配置 JAVA_HOME 环境变量。"
  exit 1
fi

#JVM启动参数
JAVA_OPTS="-Xms512m -Xmx512m -Xmn256m -XX:MetaspaceSize=128m -XX:MaxMetaspaceSize=320m"
...
#getTradeProtalPID(){
#    processes=`ps -ef|grep $APP_MAIN |grep -v "grep" |awk '{print $2}'`
#    for process in ${processes[@]}
#do
#	dir=`pwdx $process |awk '{print $2}'`
#	#echo $dir
#	if [ $dir == $APP_HOME ]
#then
#   tradePortalPID=$process
#fi
#done
#}
...
getTradeProtalPID() {
    ...
    local dir
    # 拿到所有候选 PID
    pids=$(pgrep -f "$APP_MAIN")   # 比 ps|grep|grep -v|awk 更短、更快
    [[ -z $pids ]] && return       # 一个都没找到直接返回
...
startup(){
	...
	then
        ...
        echo -n "Starting $APP_MAIN"
		    nohup java $JAVA_OPTS -classpath $CLASSPATH $APP_MAIN  > /dev/null &
        getTradeProtalPID
        if [ $tradePortalPID -ne 0 ]
		then
