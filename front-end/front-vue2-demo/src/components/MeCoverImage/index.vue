  ...
  <div class="cropper-content">
    <div class="cropper-box">
      ...
      <div class="show-preview" v-if="showPreview">
        ...
        <div>
          <span
            >图像大小：{{ parseInt(previews?.img?.width) || 0 }} px *
            {{ parseInt(previews?.img?.height) || 0 }} px</span
          >
        </div>
      </div>
    </div>
  ...
  mounted() {
    if (this.customOptions) {
      Object.keys(this.customOptions).forEach(key => {
        // 使用 Object.prototype.hasOwnProperty.call 方法替代直接调用 hasOwnProperty
        if (Object.prototype.hasOwnProperty.call(this.option, key)) {
          this.option[key] = this.customOptions[key];
        }
      });
    }
  },
