<template>
  <div class="h-full flex gap-4 p-4 bg-gray-50">
    ...
    <DictTypeEditModal
      ...
      @success="loadTypeList"
    />

    <DictItemEditModal
      v-model:visible="itemModalVisible"
      :edit-data="itemEditData"
      :current-dict-type="currentDictType"
 ...
 * DictManage - 字典管理主页面
 *
 * 采用左右分栏布局，左侧展示字典类型列表，右侧展示选中类型下的字典数据。
 * 支持类型和数据的增删改查，以及缓存刷新操作。
 *
 * @author 前端架构师
 */
...
import type { DictTypeVO, DictTypeQuery, DictItemVO, DictItemQuery } from '@/api/admin/system/dict/type'
import {
  pageDictTypes,
  deleteDictType,
  refreshDictCache,
...
import {
  dictTypeColumns,
  dictTypeSearchFields,
  dictItemColumns,
  dictItemSearchFields,
  getStatusOption
} from './data'
import DictTypeEditModal from './DictTypeEditModal.vue'
...
<style scoped>
/* 搜索表单紧凑布局 */
.dict-search-form :deep(.el-form-item) {
  margin-bottom: 0;
}
